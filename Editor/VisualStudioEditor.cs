...
namespace Microsoft.Unity.VSCodeUniversal.Editor
{
    ...
    public class VisualStudioCodeEditor : IExternalCodeEditor
    {
        CodeEditor.Installation[] IExternalCodeEditor.Installations => _discoverInstallations
            .Result
            .Values
            .Select(v => new CodeEditor.Installation() { Name = v.Name, Path = v.Path })
            .ToArray();

        private static readonly AsyncOperation<Dictionary<string, IInstallation>> _discoverInstallations;
        ...
        static VisualStudioCodeEditor()
        {
            ...
            _discoverInstallations = AsyncOperation<Dictionary<string, IInstallation>>.Run(DiscoverInstallations);
        }
        ...
        static void LegacyVisualStudioCodePackageDisabler()
        {
            ...
            {
                var editor = CodeEditor.Editor.GetCodeEditorForPath(executable);
                if (editor == null)
                    continue;
                ...
                // only disable the com.heavycaffeiner.ide.vscodeuniversal package
                var assembly = editor.GetType().Assembly;
                var assemblyName = assembly.GetName().Name;
                if (assemblyName != "Unity.VSCodeUniversal.Editor")
                    continue;
        ...
        private static Dictionary<string, IInstallation> DiscoverInstallations()
        {
            try
            {
                return Discovery
            ...
            catch (Exception ex)
            {
                ...
                return new Dictionary<string, IInstallation>();
            }
        }
        ...
        internal virtual bool TryGetVisualStudioInstallationForPath(string editorPath, bool lookupDiscoveredInstallations, out IInstallation installation)
        {
        ...
        public virtual bool TryGetInstallationForPath(string editorPath, out CodeEditor.Installation installation)
        {
            ...
            installation = new CodeEditor.Installation() { Name = vsi?.Name, Path = vsi?.Path };
            return result;
        }
        ...
        private static void RegenerateProjectFiles(IInstallation installation)
        {
        ...
        private static void SettingsButton(ProjectGenerationFlag preference, string guiMessage, string toolTip, IInstallation installation)
        {
            var generator = installation.ProjectGenerator;
        ...
        private static bool OpenFromInstallation(IInstallation installation, string path, int line, int column)
        {
